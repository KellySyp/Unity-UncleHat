...
public class DialogueController : MonoBehaviour {
	...
	//Manually sets label. I put this in for Treasure Chests
	...
	public void setDialogue(string filename){
		lines = File.ReadAllLines ("Assets/StreamingAssets/Scripts/"+filename+".txt");
	}
	...
	public void advanceDialogue(){
		...
		if (!isWaiting) {
			...
			case 1:
				...
				int rdmInt = Random.Range (0, max);
				label = "rdm" + rdmInt;
				break;
			case 2:
				//Devil
				if (GameController.seenDevil) {
					label = "Devil";
				} else {
					label = "Start";
				}
			...
			if (dialogueType == 3) {
				...
				int i = int.Parse (label.Substring (3, 1));
				if (i > max) {
					i = max - 1;
				}
				label = "dia" + (i + 1);
	...
	public void EndDialogue(){
		...
		GameController.playerInDialogue = false;
		dialogueType = 0;
		if (GameController.targetNPC.Length > 0) {
			GameController.NPCManager [GameController.targetNPC].inDialogue = false;
			GameController.NPCManager [GameController.targetNPC].canMove = true;
			GameController.targetNPC = "";
		}
	...
	void execCall(string newLine){
		...
		if (GameController.targetNPC.Length > 0) {
			label = GameController.NPCManager [GameController.targetNPC].label;
		}
		advanceDialogue ();
	}
	...
	void createButton(){
		...
		GameObject firstOption = GameObject.FindGameObjectsWithTag ("optionBtn")[0];
		eventSys.GetComponent<EventSystem> ().SetSelectedGameObject (firstOption, null);
